...
export default class Controller {
  async getClearance(req, res, next) {
    try {
  ...
  async getClearanceById(req, res, next) {
    let { id } = req.params;
    try {
      let result = await req.db.query(
        `
  ...
  async createClearance(req, res, next) {
    ...
    let { accountId, formVal, clearanceType } = req.body;
    ...
    try {
      ...
      } else if (isEmpty(files.clearance)) {
        return res.status(400).json({
          error: 400,
          message: "No files were uploaded",
        });
      ...
      let icn = files.clearance[0].path;
      ...
      let [insertRecord] = await transaction.query(
        `
        INSERT INTO brgy_clearance
        SET ?  
      `,
        [
          {
            clearanceId: uuidFile,
            accountId: accountId,
      ...
      let auditObj = {
        ...
        actionType: `UPLOAD CLEARANCE ${clearanceType}`,
        crud: "CREATE",
